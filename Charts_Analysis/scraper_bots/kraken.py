    ...
    while attempt < retries:
        try:
            conn = psycopg2.connect(
                host=KRAKEN_DB_PRICE_HOST,
                database=KRAKEN_DB_PRICE_NAME,
                user=KRAKEN_DB_PRICE_USER,
                password=KRAKEN_DB_PRICE_PASSWORD
            )
            return conn
...
def fetch_and_store_ohlcv_in_temp(cursor, symbol, temp_candle_data_table, timeframe='30m', max_retries=5):
    retries = 0
    ...
    while True:
        ...
        try:
            ...
            for symbol in markets:
                ...
                table_name = f"kraken_{symbol.replace('/', '_').lower()}_30m"
