...
def fetch_fiat_conversion_df(engine, table_name="kraken_eur_usd_1m"):
    with engine.connect() as connection:
        df = pd.read_sql_table(table_name, con=connection)
    df['timestamp'] = pd.to_datetime(df['timestamp'], unit='ms', errors='coerce')
    df = df.dropna(subset=['timestamp'])
    df = df[['timestamp', 'close']].rename(columns={'close': 'conversion_rate'})
    return df
...
def analyze_opportunities_fixed_bins(opportunity_database_uri, liquid_database_uri,
    ...
    conversion_df = fetch_fiat_conversion_df(kraken_engine)

    # Merge all
    merged_df = pd.merge(opportunity_df, base_usdt_df, on='timestamp', suffixes=('_opportunity', '_usdt'))
    merged_df = pd.merge(merged_df, conversion_df, on='timestamp', how='inner')

    # Convert USDT prices to EUR using EUR/USD rate
    merged_df['low_liquid'] = merged_df['low_usdt'] / merged_df['conversion_rate']
    merged_df['high_liquid'] = merged_df['high_usdt'] / merged_df['conversion_rate']
    ...
    merged_df['USDT Volume_opportunity'] = merged_df['EUR Volume_opportunity'] * merged_df['conversion_rate']
