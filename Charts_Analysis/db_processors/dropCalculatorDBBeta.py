...
def bin_results_v2(occurrences, bin_width, threshold):
    ...
    bins = sorted(bins)  # ensure bins go from most negative to least negative

    binned_data = []
    total_occurrences = occurrences.shape[0]

    for bin in bins:
        count = occurrences[occurrences['Low Difference (%)'] <= bin].shape[0]
        percentile = (count / total_occurrences) * 100 if total_occurrences > 0 else 0
        binned_data.append((f"≤ {bin:.2f}%", count, percentile))
...
def analyze_chart_strategy_v2(database_uri, table_name, reference_database_uri, reference_table_name, start_datetime, end_datetime, threshold, bin_width, max_diff_pct):
    ...
    print(f"{'Bin':<12} {'Occurrences':<12} {'Percentile':<10}")
    for bin_label, count, percentile in binned_data:
        if count > 0:
            print(f"{bin_label:<12} {count:<12} {percentile:.2f}%")
...
if __name__ == "__main__":
    ...
    bin_width = 0.01  # Bin width for percentage drops
