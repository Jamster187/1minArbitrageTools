...
def filter_bad_candles(df, max_diff_pct):
    # Exclude candles where the primary market high exceeds the reference market high by max_diff_pct
    df = df[(df['high'] <= df['high_ref'] * (1 + max_diff_pct / 100))]
    return df

def count_occurrences_v2(df, threshold):
    # Count rows where the high difference exceeds the threshold
    return df[df['High Difference (%)'] >= threshold]

def bin_results_v2(occurrences, bin_width, threshold):
    # Bin the occurrences based on high percentage changes
    bins = np.arange(threshold, occurrences['High Difference (%)'].max() + bin_width, bin_width)
    binned_data = {f"≥ {bin:.2f}%": occurrences[occurrences['High Difference (%)'] >= bin].shape[0] for bin in bins}
    return binned_data
...
def analyze_chart_strategy_v2(database_uri, table_name, reference_database_uri, reference_table_name, start_datetime, end_datetime, threshold, bin_width, max_diff_pct):
    ...
    # Exclude candles where high exceeds the reference market high by max_diff_pct
    ...
    print("\nOccurrences of Percentage Changes:")
...
if __name__ == "__main__":
    opportunity_exchange_string = 'Bitget'
    liquid_exchange_string = 'Bitget'
    base_asset = 'ani'
    opportunity_quote_asset = 'usdt'
    ...
    database_uri = f"postgresql+psycopg2://postgres:password@localhost:5432/Testing_Data_Collection_{liquid_exchange_string}"
    reference_database_uri = f"postgresql+psycopg2://postgres:password@localhost:5432/Testing_Data_Collection_{opportunity_exchange_string}"
    ...
    start_datetime = pd.to_datetime("2024-09-01 00:00:00")
    end_datetime = pd.to_datetime("2026-12-30 23:59:59")
    threshold = 0.1  # Minimum percentage drop threshold
