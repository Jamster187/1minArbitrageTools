...
def bin_results(occurrences, bin_width, threshold):
    bins = np.arange(threshold, occurrences['percentage_change'].min() - bin_width, -bin_width)
...
def main(file_path, start_datetime, end_datetime, threshold, bin_width, max_diff_pct):
    ...
    #binance_alt_usdt, kucoin_safe_usdt, kucoin_ethfi_usdt, kucoin_zeta_usdt, kucoin_ena_usdt, bitget_arkm_usdt

if __name__ == "__main__":
    file_path = 'kraken_pump_usd, 1s.csv'
    start_datetime = pd.to_datetime('2025-01-10 00:00:00')
    end_datetime = pd.to_datetime('2026-11-30 23:59:59')
