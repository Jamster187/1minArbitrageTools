...
import time
from config import kraken_arbitrage1_websocket  # Using kraken_arbitrage1_websocket from config
from dbHelpers import *
from market_settings import *

# Database configuration
DBHOST = KRAKEN_DB_HOST
DBNAME = KRAKEN_DB_NAME
DBUSER = KRAKEN_DB_USER
DBPASS = KRAKEN_DB_PASSWORD
...
async def reconnect_on_timeout(exchange_object, base_asset, target_quote_asset, conn, TABLE_NAME, depth):
    ...
    exchange_object = ccxtpro.kraken()  # Reinitialize kraken_arbitrage1_websocket object
...
# Main function
async def main(base_asset, target_quote_asset, target_exchange_name_string_for_db, depth=500):
    # Connect to database
    conn = connect_db()
    ...
    try:
        await fetch_order_book_websocket(kraken_arbitrage1_websocket, base_asset, target_quote_asset, conn, TABLE_NAME, depth)
