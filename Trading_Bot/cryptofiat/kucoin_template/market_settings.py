...
base_asset='REEF' # this is the base asset symbol e.g 'SOL', 'BTC'
target_quote_asset='USDT' # this is the target quote asset symbol, i.e on kraken it is USD or EUR often
...
min_sell_premium_list = [0.1] # this is how much we sell for a premium - negative value means we are paying the market some % to get us out quickly. 0.01 = 1%
max_base_asset_to_use = 69000 # this the max amount of the base asset we can ever have allocated at once to a particular market. this will take into account the already spent assets so that we do not exceed this budget
...
min_profitable_discount_list = [0.035, 0.037, 0.04, 0.042, 0.046, 0.05, 0.052, 0.069, 0.089, 0.094, 0.095] #this is how much of a discount we are asking for from the market. 0.01 = 1%
...
sell_closing_discount = -0.005 # this is how much we sell for a premium - negative value means we are paying the market some % to get us out quickly. 0.01 = 1%
sell_close_sleep_time = 2.5 # time the bot pauses for before repeating loop

# Price 1
price_pusher_1_base_asset='REEF'
